...
export default function Home() {
  const [usuarios, setUsuarios] = useState([]);
  const [modalVisible, setModalVisible] = useState(false);
  const [titulo, setTitulo] = useState("");  // Estado para o título da notificação
  const [mensagem, setMensagem] = useState("");
  const [userSelected, setUserSelected] = useState(null);
  const [expoPushToken, setExpoPushToken] = useState("");
  const notificationListener = useRef();
  ...
  async function enviarNotificacao() {
    ...
    if (!titulo.trim()) {
      Alert.alert("Digite o título da notificação");
      return;
    }
    ...
    try {
      const payload = {
        to: userSelected.tokenexpo,
        sound: "default",
        title: titulo,  // Enviar o título junto com a mensagem
        body: mensagem,
      };
      ...
      Alert.alert("Enviado!", "Notificação enviada para " + userSelected.nome);
      setTitulo("");  // Limpar o campo de título
      setMensagem("");  // Limpar o campo de mensagem
      setModalVisible(false);
    } catch (error) {
      console.log("Erro ao enviar notificação:", error.response || error);
      Alert.alert(
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalFundo}>
          <View style={styles.modal}>
            ...
            {/* Campo para digitar o título da notificação */}
            <TextInput
              style={styles.input}
            ...
            {/* Campo para digitar a mensagem da notificação */}
            <TextInput
              style={styles.input}
              placeholder="Digite a mensagem"
              value={mensagem}
